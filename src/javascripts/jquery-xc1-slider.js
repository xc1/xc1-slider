...
(function($) {
	...
	$.xc1Slider = function(slider, options) {
		...
		slider.fn.show = function(slide) {
			...
			if((slider.vars.current <= 0 && slider.settings.direction == 'backward') || (slide >= slider.vars.slide.length && slider.settings.direction == 'forward') || (slider.vars.pos <= slider.vars.min && slider.settings.direction == 'backward' && slider.settings.effect == 'scroll') || (slider.vars.pos >= slider.vars.max && slider.settings.direction == 'forward' && slider.settings.effect == 'scroll')) { slider.fn.reset(); }
		...
		slider.fn.current = function()  {
			...
			if(slider.settings.effect == 'scroll') {
				for(var i = 0; i < slider.vars.slide.length; i++) {				
					if(slider.vars.pos > slider.vars.slide[i].left) {
